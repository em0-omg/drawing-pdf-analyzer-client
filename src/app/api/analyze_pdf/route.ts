...
export async function POST(request: NextRequest) {
  try {
    ...
    const files = Array.from(formData.entries()).filter(([key, value]) => 
      value instanceof File
    );
    console.log(`[analyze_pdf] Files in form data: ${files.length} files`);
    files.forEach(([key, file]) => {
      console.log(`[analyze_pdf] File: ${key} = ${(file as File).name} (${(file as File).size} bytes)`);
    });
    ...
    console.log(`[analyze_pdf] Backend response: ${response.status} ${response.statusText}`);

    if (!response.ok) {
      const errorText = await response.text();
      console.error(`[analyze_pdf] Backend error: ${response.status} - ${errorText}`);
    ...
    console.log(`[analyze_pdf] Success: returning data with keys: ${Object.keys(data).join(', ')}`);
